...
export function validate(calendar: Calendar): {
	...
	for (const [index, term] of calendar.terms.entries()) {
		...
		for (const special of term.dates) {
			if (!special.date) {
				...
				if (special.date < term.start) {
					errors.push(`Term ${termName}: special date ${special.date} before term start`)
				} else if (special.date > term.end) {
					errors.push(`Term ${termName}: special date ${special.date} after term end`)
				}
				if (
					special.type === "follow" &&
		...
		for (const course of term.courses) {
			...
			if (!course.number) {
				errors.push(`Term ${termName}, course ${courseName}: missing/invalid number`)
			}
			if (!course.name) {
				errors.push(`Term ${termName}, course ${courseName}: missing/invalid name`)
			}
			...
			for (const mp of course.meetingPatterns) {
				...
				if (!mp.endTime) {
					errors.push(`Term ${termName}, course ${courseName}: missing/invalid end time`)
				}
				...
				if (mp.weekdays.length === 0) {
					errors.push(`Term ${termName}, course ${courseName}: no weekdays selected`)
				}
				if (!mp.location) {
					warnings.push({
