...
export function parseMeetingPattern(pattern: string): MeetingPattern | Error {
	...
	if (times.length !== 2) {
		return new Error(`scanning MeetingPattern: expected two times, got ${times.length}`)
	}
	const startTime = parseTime(times[0])
	const endTime = parseTime(times[1])
	if (startTime instanceof Error || endTime instanceof Error) {
		return new Error(`scanning MeetingPattern: failed to parse time: ${startTime} ${endTime}`)
	}
