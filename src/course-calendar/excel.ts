...
> {
	...
	const sheet = await sheetToArray(fixSheetRef(workbook.Sheets[workbook.SheetNames[0]]))

	// Sanity check: A1 is either "View My Courses" or "My Enrolled Courses"
	// in Workday exports
	if (sheet[0][0] !== "View My Courses" && sheet[0][0] !== "My Enrolled Courses") {
		return {
			error: new Error(
	...
	for (const [i, row] of sheet.entries()) {
		...
		if (parseIndices) {
			...
			if (Object.values(requiredIndices(index)).some((v) => v == null || v == -1)) {
				...
				console.warn(index)
				warnings.push({
					title: `Possibly missing data for ${sectionType} section at row ${i + 1}`,
		...
		if (
			...
			console.warn(
				`Warning: row ${i + 1}: missing values for section ${sectionType}, skipping`,
			)
			console.warn(row)
			console.warn(index)
			warnings.push({
		...
		if (!calendar) {
			...
			if (!calendar) {
				console.warn(`Warning: no matching calendar found, creating blank calendar`)
				calendar = {
		...
		if (registrationStatus !== "Registered" && registrationStatus !== "Completed") {
			console.debug(
		...
		if (term) {
			...
			console.warn(`Warning: no matching term found, can't process course ${courseSection}`)
			warnings.push({
		...
		if (meetingPatterns.some((err) => err instanceof Error)) {
			console.warn(`Warning: failed to parse meeting patterns for course ${courseSection}`)
	...
	for (const s of pendingSubsections) {
		...
		if (!courses.length) {
			console.warn(`Warning: course ${s.courseNameNumber} not found in term ${s.term.id}`)
			warnings.push({
