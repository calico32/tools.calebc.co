...
export class Generator implements AlpineComponent<Generator> {
	...
	readonly hasNextYearCalendar = !!nextCalendar
	readonly currentAY = currentCalendar
	readonly currentSummer = currentSummerCalendar.replace("-", " ")
	readonly nextAY = nextCalendar
	readonly nextSummer = nextSummerCalendar?.replace("-", " ") ?? null
	...
	async importIcs(this: AlpineThis<Generator>): Promise<void> {
		...
		if (!Array.isArray(cal) || cal[0] !== "vcalendar" || !cal[1] || !Array.isArray(cal[1])) {
			toaster.show({
				message: "Invalid .ics file.",
				class: "error",
			})
		...
		for (const [id, , , value] of cal[1]) {
			if (id === "prodid") {
				...
				toaster.show({
					message: "Restore succeeded. Review and make any necessary changes below.",
					class: "success",
				})
				return
			}
		...
		;(this.$refs.icsInput as HTMLInputElement).value = ""
		toaster.show({
			message: "Couldn't find calendar data in the .ics file. Please try again.",
			class: "error",
		})
	}
...
/**
 * decodeCalendar decodes a calendar object from a string via base64url decoding > Gunzip > JSON
 * decoding.
 */
...
declare global {
	interface Uint8Array {
		toBase64(options?: {
			alphabet?: "base64" | "base64url" | "base64pad" | "base64urlpad"
			omitPadding?: boolean
		}): string
	}
