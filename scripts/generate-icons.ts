	...
	await Promise.all(
		icons.map(async ([size, type, purpose]) => {
			if (type === "favicon") {
				...
				await fs.writeFile(path.resolve(__dirname, "../assets", "favicon.ico"), ico)

				return
			}

			const buffer = drawIcon(size, purpose === "maskable").toBuffer(("image/" + type) as any)

			const name = purpose === "maskable" ? `${size}-maskable.${type}` : `${size}.${type}`

			await fs.writeFile(path.resolve(__dirname, "../assets/icon", name), buffer)
		}),
	)
}
